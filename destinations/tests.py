...
from destinations.api.v1.client.views import AutogeneratedMessageAPIView
...
class AutogeneratedMessageApiTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(email="prompt@example.com", password="testpass123")

    @patch("destinations.api.v1.client.views.random.choice")
    def test_returns_preferred_destination_in_autogenerated_message(self, random_choice_mock):
        beach = self._create_destination(
            "Cox's Bazar",
            "BGD",
            destination_type=DestinationType.BEACH,
        )
        self._create_destination("Kathmandu", "NPL", destination_type=DestinationType.CITY)
        self.user.profile.travel_interests = ["beach"]
        self.user.profile.save(update_fields=["travel_interests"])
        self.client.force_authenticate(user=self.user)
        random_choice_mock.side_effect = [
            AutogeneratedMessageAPIView.GENERAL_TEMPLATES[0],
            beach.id,
        ]

        response = self.client.get("/api/v1/destinations/autogenerated-message/")

        self.assertEqual(response.status_code, drf_status.HTTP_200_OK)
        self.assertEqual(response.data["data"]["destination_name"], beach.name)
        self.assertEqual(response.data["data"]["destination_slug"], beach.slug)
        self.assertIn(beach.name, response.data["data"]["message"])

    @patch("destinations.api.v1.client.views.random.choice")
    def test_seasonal_message_uses_current_month_destination(self, random_choice_mock):
        current_month = timezone.localdate().month
        seasonal_destination = self._create_destination(
            "Bali",
            "IDN",
            destination_type=DestinationType.ISLAND,
            best_travel_months=[current_month],
        ...
        random_choice_mock.side_effect = [
            AutogeneratedMessageAPIView.GENERAL_TEMPLATES[1],
            True,
            seasonal_destination.id,
        ]

        response = self.client.get("/api/v1/destinations/autogenerated-message/")

        self.assertEqual(response.status_code, drf_status.HTTP_200_OK)
        self.assertEqual(response.data["data"]["destination_name"], seasonal_destination.name)
        self.assertTrue(response.data["data"]["is_now_best_time"])
        self.assertIn(timezone.localdate().strftime("%B"), response.data["data"]["message"])

    def test_returns_fallback_message_when_no_destination_exists(self):
        response = self.client.get("/api/v1/destinations/autogenerated-message/")

        self.assertEqual(response.status_code, drf_status.HTTP_200_OK)
        self.assertEqual(response.data["data"]["destination_name"], "")
        self.assertEqual(response.data["data"]["destination_slug"], "")
        self.assertIn("Tell me what kind of trip you want", response.data["data"]["message"])
    ...
    def _create_destination(
        ...
        *,
        ...
        return Destination.objects.create(
            ...
            budget_tier=BudgetTier.MID,
            best_travel_months=best_travel_months or [],
